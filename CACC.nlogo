;; suppose 1 tick = 0.1 s
;; suppose 1 cor = 5 m
;; hence 1 cor/tick = 5 m/ 0.1 s = 50 m/s, 0.1 cor/tick = 5 m/s
;; and 0.01 cor/tick2 = 5 m/s2

globals [ sample-car front-cars PV distance-of-PV thw max-acceleration-of-CACC/ACC max-deceleration-of-CACC/ACC comfortable-acceleration max-deceleration xk xk-1 vk vk-1 vkpre Tdeta Tdc Tde Vp Pp Vf Pf Lp
  A1obj_lower A1obj_upper A2obj_lower A2obj_upper A3obj_lower A3obj_upper Aobj_lower Aobj_upper
  head-car other-cars other-cars-2 other-cars-3 other-cars-4 second-car third-car fourth-car fifth-car]

turtles-own [acceleration pre-speed speed next-speed next-dis next-dis-in-netlogo speed-upper-limit speed-lower-limit min-speed max-speed]
...
to setup
  ...
  setup-cars
  set thw 1.1
  set comfortable-acceleration 2
  set max-deceleration -6
  set max-acceleration-of-CACC/ACC 1
  set max-deceleration-of-CACC/ACC -2.8
  set Tdeta 0.1
  set Tdc 0.1
  set Tde 0.1
  set Lp 5
...
to setup-cars
  ...
  create-turtles number-of-cars [
    set color blue
    set xcor 2 * random 50
    set heading 90
    ;; set initial speed
    set speed 10 + random-float 5
    set speed-upper-limit 18
    set speed-lower-limit 10
    set max-speed 20
    set min-speed 0
    separate-cars]
  ...
  set head-car max-one-of turtles [xcor]
  set other-cars turtles with [xcor < [xcor] of head-car]
  ask turtles [set next-speed speed]
...
to follow-PV-CACC
  ...
  set pre-speed speed
  set xk xcor * 5
  set xk-1 [xcor * 5] of PV
  set vk speed
  set vk-1 [speed] of PV
  set acceleration (vkpre + 0.45 *(xk-1 - xk) + 0.25 * vk-1 - (1 + 0.25 + 0.45 * thw) * vk) / (0.25 * thw)

  ifelse acceleration > max-acceleration-of-CACC/ACC
  [set acceleration max-acceleration-of-CACC/ACC]
  [if acceleration < max-deceleration-of-CACC/ACC
    [set acceleration max-deceleration-of-CACC/ACC]]
...
to-report any-PV?
  let myhd heading
  set front-cars turtles with [(distance myself) != 0 and (towards myself) != myhd]
  set PV (min-one-of front-cars [distance myself])
  ifelse PV != nobody
  [set distance-of-PV  distance PV
    ifelse distance-of-PV <= 500
    [report true]
    [report false]]
  [report false]
end
...
to speed-fluctuate
  set pre-speed speed
  set acceleration  random-float (2 * max-acceleration-of-CACC/ACC) - max-acceleration-of-CACC/ACC

  ;;calculate the distance travelled in this cycle
  ifelse speed + acceleration * Tdeta < speed-lower-limit
  [let t-deceleration (speed-lower-limit - speed) / acceleration
   set next-dis speed * t-deceleration + 0.5 * acceleration * t-deceleration * t-deceleration + speed-lower-limit * (Tdeta - t-deceleration)]
  [ifelse speed + acceleration * Tdeta > speed-upper-limit
    [let t-acceleration (speed-upper-limit - speed) / acceleration
     set next-dis speed * t-acceleration + 0.5 * acceleration * t-acceleration * t-acceleration + speed-upper-limit * (Tdeta - t-acceleration)]
    [set next-dis speed * Tdeta + 0.5 * acceleration * Tdeta * Tdeta]]

  ;;update the speed at the beginning of next cycle
  ifelse speed + acceleration * Tdeta < speed-lower-limit
  [set next-speed speed-lower-limit]
  [ifelse speed + acceleration * Tdeta > speed-upper-limit
    [set next-speed speed-upper-limit]
    [set next-speed speed + acceleration * Tdeta]]
...
"" ""
PENS
"a1" 1.0 0 -16777216 true "" "plot [acceleration] of turtle 0"
"v1" 1.0 0 -7500403 true "" "plot [speed] of turtle 0"
"x1" 1.0 0 -2674135 true "" "plot [xcor] of turtle 0"
"a2" 1.0 0 -955883 true "" "plot [acceleration] of turtle 1"
"v2" 1.0 0 -6459832 true "" "plot [speed] of turtle 1"
"x2" 1.0 0 -1184463 true "" "plot [xcor] of turtle 1"
...
"" ""
PENS
"a1" 1.0 0 -2674135 true "" "plot [acceleration] of turtle 0"
"a2" 1.0 0 -13840069 true "" "plot [acceleration] of turtle 1"
...
"" ""
PENS
"default" 1.0 0 -2674135 true "" "plot [speed] of turtle 0"
"pen-1" 1.0 0 -13840069 true "" "plot [speed] of turtle 1"
